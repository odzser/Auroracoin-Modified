...
json_spirit::Value GetNetworkHashPS(int lookup, int height, int algo) {
    CBlockIndex *pb = GetLastBlockIndexForAlgo(chainActive.Tip(), algo);

    if (height >= 0 && height < chainActive.Height())
        pb = GetLastBlockIndexForAlgo((CBlockIndex*) chainActive[height], algo);
    ...
    for (int i = 0; i < lookup; i++) {
        pb0 = GetLastBlockIndexForAlgo((CBlockIndex*) pb0->pprev, algo);
        if (pb0 == NULL) break;
...
json_spirit::Value getnetworkhashps(const json_spirit::Array& params, bool fHelp)
{
    ...
    return GetNetworkHashPS(params.size() > 0 ? params[0].get_int() : 120, params.size() > 1 ? params[1].get_int() : -1, miningAlgo);
}

json_spirit::Value getallnetworkhashps(const json_spirit::Array& params, bool fHelp)
{
    if (fHelp || params.size() > 2)
        throw std::runtime_error(
    ...
    int blocks = params.size() > 0 && !params[0].is_null() ? params[0].get_int() : 360;
    int height =  params.size() > 1 && !params[1].is_null() ? params[1].get_int() : -1;

    json_spirit::Object obj;
    obj.push_back(json_spirit::Pair("groestl", GetNetworkHashPS(blocks, height, ALGO_GROESTL)));
    obj.push_back(json_spirit::Pair("qubit", GetNetworkHashPS(blocks, height, ALGO_QUBIT)));
    obj.push_back(json_spirit::Pair("scrypt", GetNetworkHashPS(blocks, height, ALGO_SCRYPT)));
    obj.push_back(json_spirit::Pair("sha256d", GetNetworkHashPS(blocks, height, ALGO_SHA256D)));
    obj.push_back(json_spirit::Pair("skein", GetNetworkHashPS(blocks, height, ALGO_SKEIN)));
    return obj;
}
...
json_spirit::Value getblocktemplate(const json_spirit::Array& params, bool fHelp)
{
    if (fHelp || params.size() > 2)
        throw std::runtime_error(
            "getblocktemplate ( \"jsonrequestobject\" ) ( algorithm )\n"
    ...
    int templateAlgorithm = !algorithm.empty() ? GetAlgoByName(algorithm) : miningAlgo;
    ...
    if (pindexPrev != chainActive.Tip() ||
        ...
        (pblock != NULL && pblock->GetAlgo() != templateAlgorithm))
    {
        ...
        pblocktemplate = CreateNewBlock(scriptDummy, templateAlgorithm);
        if (!pblocktemplate)
            throw JSONRPCError(RPC_OUT_OF_MEMORY, "Out of memory");
