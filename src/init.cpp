...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    std::string strDataDir = pathDataDir.string();
    ...
    if (!fDisableWallet) {
        ...
        if (!bitdb.Open(pathDataDir))
        {
            // try moving the database env out of the way
            boost::filesystem::path pathDatabase = pathDataDir / "database";
            boost::filesystem::path pathDatabaseBak = pathDataDir / strprintf("database.%d.bak", GetTime());
            try {
        ...
        if (boost::filesystem::exists(pathDataDir / strWalletFile))
        {
    ...
    boost::filesystem::path blocksDir = pathDataDir / "blocks";
    if (!boost::filesystem::exists(blocksDir))
    {
        boost::filesystem::create_directories(blocksDir);
        bool linked = false;
        for (unsigned int i = 1; i < 10000; i++) {
            boost::filesystem::path source = pathDataDir / strprintf("blk%04u.dat", i);
